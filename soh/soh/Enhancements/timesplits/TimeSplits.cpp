...
void TimeSplitsPopUpContext() {
    if ((popupID != -1) && ImGui::BeginPopup("TimeSplitsPopUp")) {
        if (popupID == ITEM_SKULL_TOKEN) {
            ...
            ImGui::PopItemWidth();
            if (ImGui::Button(SohGui::TrLabel("Set Tokens").c_str())) {
            ...
            for (auto item : popupList[popupID]) {
                ...
                if (popupObject.splitType == SPLIT_TYPE_UPGRADE) {
                    if (popupID <= ITEM_SLINGSHOT && popupID != -1) {
                        ...
                        ImGui::Text("%s", SohGui::Tr(upgOutput).c_str());
                    }
                }
                ImGui::EndGroup();
                if (rowIndex != 5) {
...
void TimeSplitsDrawSplitsList() {
    ...
    if (ImGui::BeginTable("Splits", 5, ImGuiTableFlags_Hideable | ImGuiTableFlags_Reorderable)) {
        ...
        for (auto& split : splitList) {
            ...
            ImGui::TextWrapped("%s", SohGui::Tr(split.splitName).c_str());
            ImGui::TableNextColumn();
            // Current Time
    ...
    for (auto& split : splitObjectList) {
        if (split.splitType == type) {
            ...
            if (type > SPLIT_TYPE_QUEST) {
                ...
                ImGui::AlignTextToFramePadding();
                ImGui::Text("%s", SohGui::Tr(split.splitName).c_str());
                ImGui::PopStyleVar(1);
            }
        }
    }
...
void TimeSplitsDrawOptionsMenu() {
    ImGui::SeparatorText(SohGui::Tr("Window Options").c_str());
    ...
    ImGui::SeparatorText(SohGui::Tr("Split List Management").c_str());

    ImGui::TextUnformatted(SohGui::Tr("New List Name: ").c_str());
    ImGui::PushItemWidth(150.0f);
    ...
    ImGui::TextUnformatted(SohGui::Tr("Select List to Load: ").c_str());
    ImGui::PushItemWidth(150.0f);
...
void TimeSplitsDrawManageList() {
    ...
    if (ImGui::BeginTable("List Management", 2, ImGuiTableFlags_BordersInnerV)) {
        ...
        if (ImGui::BeginTabItem(SohGui::TrLabel("Preview").c_str())) {
        ...
        if (ImGui::BeginTabItem(SohGui::TrLabel("Quest").c_str())) {
            TimeSplitsDrawItemList(SPLIT_TYPE_QUEST);
            ImGui::EndTabItem();
        }
        if (ImGui::BeginTabItem(SohGui::TrLabel("Entrances").c_str())) {
            TimeSplitsDrawItemList(SPLIT_TYPE_ENTRANCE);
            ImGui::EndTabItem();
        }
        if (ImGui::BeginTabItem(SohGui::TrLabel("Bosses").c_str())) {
            TimeSplitsDrawItemList(SPLIT_TYPE_BOSS);
            ImGui::EndTabItem();
        }
        if (ImGui::BeginTabItem(SohGui::TrLabel("Miscellaneous").c_str())) {
...
void TimeSplitWindow::DrawElement() {
    ...
    if (ImGui::BeginTabBar("Split Tabs")) {
        if (ImGui::BeginTabItem(SohGui::TrLabel("Splits").c_str())) {
            TimeSplitsDrawSplitsList();
            ImGui::EndTabItem();
        }
        if (ImGui::BeginTabItem(SohGui::TrLabel("Manage List").c_str())) {
